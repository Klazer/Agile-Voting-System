...
public class Election {
    ...
    /**
     * This adds all the votes from the in the input parameter add election
     * into the election it is called on. This essentaily combines elections
     * allowing 2 elections made from 2 election files to be made into one
     * Election. This can be chained to allow an election of an arditary amount
     * of election files.
     *
     * @return A Candidate array containing information for each candidate
     *
     */
    public void combine(Election add){
        int total = this.ballots.length + add.ballots.length;
        Ballot votes[] = new Ballot[total];
        for(int i = 0; i < this.ballots.length; i++){
            votes[i] = this.ballots[i];
        }
        for(int i = this.ballots.length; i < total; i++){
            votes[i] = add.ballots[i-this.ballots.length];
        }
        this.ballots = votes;
    }
